...
public class DAI_PeopleListCommand implements DAI_Command{
	...
	public void execute(Model model) {
		...
		String PpageNum = (String)session.getAttribute("PpageNum");
		...
		// 현재 페이지의 수 
		int currentPage = Integer.parseInt(PpageNum);
		...
		ArrayList<DAI_Dto>dtos = dao.DAI_plist(StrRow, EndRow);
		
		model.addAttribute("people_list",dtos);
		model.addAttribute("PStartPage", startPage);
		model.addAttribute("PEndPage", EndPage);
		model.addAttribute("PPageCount", PageCount);
		model.addAttribute("PPageNum", PpageNum);
